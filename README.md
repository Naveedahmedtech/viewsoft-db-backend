...
## Sharing multi-database support with older versions

If you are preparing a shorter-lived or older version of this project that currently only knows PostgreSQL, give the other maintainer the following files so they can reuse the multi-database wiring:

- `server.js`
- `app/models/index.js`
- `app/config/db.config.js`
- `.env` (or a copy of the database section with the new `DATABASE_TYPE` value)

Pair those files with `MULTI_DATABASE_SUPPORT.md` in this repo so they can see the configuration hints, and make sure they install `dotenv` (`npm install dotenv` or add it to `package.json`) before starting the app; otherwise `server.js` and the models cannot read the `.env` file.
